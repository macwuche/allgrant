                                        ...
                                        <select name="gateway_code" class="box-input deposit-methods" id="gatewaySelect">
                                            ...
                                            @if(!setting('multiple_currency','permission'))
                                                @foreach ($gateways as $gateway)
                                                    <option data-logo="{{ $gateway->logo ? asset($gateway->logo) : $gateway->gateway_logo }}" value="{{ $gateway->gateway_code }}">{{ $gateway->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
...
@push('style')
    <style>
        /* Self-contained styling for admin-inserted Payment Details copy buttons — not
           relying on `.site-btn-xs`/`.primary-btn` (admin-panel classes, not defined in
           this theme's CSS). Picks up the theme's own --td-primary accent. */
        .copy-clipboard-btn {
