...
class DashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $browser = Cache::remember('login-browser-stats', 60, function () {
            $browserCounts = [];

            LoginActivities::select(['id', 'agent'])
                ->chunk(1000, function ($activities) use (&$browserCounts) {
                    foreach ($activities as $activity) {
                        $browser = $activity->browser; // computed attribute
        ...
        $platform = Cache::remember('login-platform-stats', 60, function () {
            $platformCounts = [];

            LoginActivities::select(['id', 'agent'])
                ->chunk(1000, function ($activities) use (&$platformCounts) {
                    foreach ($activities as $activity) {
                        $platform = $activity->platform; // computed attribute
