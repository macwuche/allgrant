...
class IpnController extends Controller
{
    ...
    public function flutterwaveIpn(Request $request)
    {
        if ($request->has('status')) {
            ...
            } elseif (in_array($request->input('status'), ['successful', 'completed'])) {
                $txid = $request->input('transaction_id');
                $response = Http::withHeaders([
                    'Content-Type' => 'application/json',
                    'Authorization' => 'Bearer ' . $credentials->secret_key,
                ])->get(sprintf('https://api.flutterwave.com/v3/transactions/%s/verify', $txid));
                if ($response->successful()) {
                    $res = $response->json();
                    $amountPaid = $res['data']['amount'];
                    $amountToPay = (float) $res['data']['meta']['price'];

                    if ($amountPaid >= $amountToPay) {
                        return self::paymentSuccess($txnid, true);
                    }
    ...
    public function monnifyIpn()
    {
        ...
        if (htmlspecialchars($_GET['paymentReference'])) {
            ...
            if ($verify['paymentStatus'] == 'PAID') {
                return self::paymentSuccess($ref, true);
            }
